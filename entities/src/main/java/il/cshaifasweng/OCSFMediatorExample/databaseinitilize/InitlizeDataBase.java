...
//		session.getTransaction().commit();
	...
	public static String getAllQuestionsAnswers() throws Exception {
		...
		for (Question question : questions) {
			...
			for (Answer answer : answers) {
				answerNum++;
				answersStrings = answersStrings + "	- " + answerNum + ") " + answer.getAnswer() + "    -id:("
						+ answer.getId() + ")" + "\n";
			}
			answerNum = 0;
	...
	public static void AddSpecificquestion(Answer answer) {
		System.out.println("3eeeeeeeeeeeeee");
		session.beginTransaction();
		Answer qanser = new Answer();
		qanser.setAnswer(answer.getAnswer());
		qanser.setQuestion(answer.getQuestion());
		System.out.println("BUR3eeeeeeeeeeeeee");
		System.out.println(qanser.getAnswer());
